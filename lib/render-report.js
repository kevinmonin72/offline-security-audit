...
function renderHtmlReport(reportsArray) {
    ...
    if (validFindings.length === 0) {
        ...
        validFindings.forEach(f => {
            ...
            // Ultra vulgarisation (impact business / légal)
            let vulgarised = f.description || "Écart de sécurité ou exposition d'en-tête identifié.";
            if (sev === "Critical" || sev === "High") {
                vulgarised = `🚨 <strong>Danger Business Immédiat :</strong> ${f.description || "Cette brèche permet à un attaquant d'intercepter des sessions clients, de voler des cookies ou d'injecter du contenu malveillant. Sanctions RGPD encourues (Art. 32)."}`;
            } else if (sev === "Medium") {
                vulgarised = `⚠️ <strong>Vulnérabilité SEO & Réputation :</strong> ${f.description || "Absence de directive de durcissement facilitant l'usurpation d'identité du site et dégradant l'indice de confiance Google."}`;
            }

            // Remédiation ultra-technique brute
            ...
            <div class="finding-card" style="border-left-color: ${sCol}">
                ...
                <div style="margin-top:14px;display:inline-block;background:#0284c725;color:#38bdf8;border:1px solid #0284c7;padding:5px 14px;border-radius:8px;font-size:0.78rem;font-weight:700;letter-spacing:0.3px;">
                    ⚡ Confirmé Réel par Contre-Vérification Active Serveur
                </div>
            </div>`;
        });
    }
